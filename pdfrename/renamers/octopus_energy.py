...
def statement(document: pdf_document.Document) -> NameComponents | None:
    ...
    #
    # Note that some time ago (2023), Octopus used the wrong producer,
    # so this should try to grab those, as well.
    producer = document.producer or b""
    if (
        b"kraken-tech-" not in producer
        and b"octopusenergy-" not in producer
        and b"--help" not in producer
    ):
        return None
